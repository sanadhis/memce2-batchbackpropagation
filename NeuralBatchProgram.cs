...
namespace BatchTrain
{
    class NeuralBatchProgram
    {
        static void Main(string[] args)
        {
            ...
            //StoreToDatabase(weights);

            //read newest data from sql to perform feedfoward      
            string newestData = System.IO.File.ReadAllText(@"C:\csharp\dataUpdate.txt");
            double[] newData = new double[6];
            int k = 0;
            foreach (string data in newestData.Split(' '))
            {
                double.TryParse(data, NumberStyles.Number, CultureInfo.InvariantCulture, out newData[k++]);
                if (k == 6)
                    break;
            }

            foreach (double weight in getFromWeight())
            {
                Console.WriteLine(weight);
            }
            // Console.WriteLine("\nTes feedfoward = " + nn.feedfoward(newData));

            // Console.WriteLine(newestData);

            // for (int counter=0;counter<=149;counter++)
            // {
            //     Console.WriteLine("\n" + counter + " Tes feedfoward = " + nn.feedfoward(allData[counter]));
            // }
        ...
        static void StoreToDatabase(double[] dataWeight)
        {
            string cs = @"server=localhost;Uid=root;
          Pwd=root;database=gemastik2015_memce2";
            ...
            finally
            {
                if (conn != null)
                {
                    ...
                    mSqlCmdSelectCustomers.CommandText = "insert into weightNN values";
                    foreach (double weight in dataWeight)
                    {
                        mSqlCmdSelectCustomers.CommandText += "(" + weight + "),";
                    }

                    mSqlCmdSelectCustomers.CommandText = mSqlCmdSelectCustomers.CommandText.Remove(mSqlCmdSelectCustomers.CommandText.Length - 1);
                    MySqlDataReader mSqlExecute;
                    mSqlExecute = mSqlCmdSelectCustomers.ExecuteReader();
                }
            }
        }

        static double[] getFromWeight()
        {
            string cs = @"server=localhost;Uid=root;
          Pwd=root;database=gemastik2015_memce2";
            double[] weights = null;
            ...
            finally
            {
                ...
                {
                    MySqlCommand mSqlCmdSelectCustomers = conn.CreateCommand();
                    mSqlCmdSelectCustomers.CommandText = "select * from weightNN";
                    MySqlDataReader mSqlReader_Customers;
                    mSqlReader_Customers = mSqlCmdSelectCustomers.ExecuteReader();
                    weights = new double[59];

                    int counter = 0;
                    while (mSqlReader_Customers.Read())
                    {
                        weights[counter++] = mSqlReader_Customers.GetDouble(0);
                    }
                }
            }
            return weights;
